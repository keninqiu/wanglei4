...
+function ($, window) {
    ...
    $.fn.FixFill = function (maxWidth, maxHeight, redirect) {
        var img = new Image();
        ...
        wzArticleCrumbs = function () {
            ...
            this.init = function (opt) {
                ...
                if (referrer != undefined && referrer != "" && referrer.indexOf(origin) > -1) {
                    var referrerPathname = referrer.substring(origin.length, referrer.length).toLowerCase();
                    if (referrerPathname.indexOf("/") === 0) {
                        referrerPathname = referrerPathname.substring(1, referrerPathname.length);
                    }
                    var reg = RegExp(/^\w*[a-zA-Z]+\w*$/);
                    var _self = this;
                    var languageId = this.options.languageId;
                    var hisHeight = $('#' + this.options.controlId + ' .w-crumbs').height();
                    var homeItemTitle = $('#' + this.options.controlId + ' .w-crumbs-item:first').text();
                    if (referrerPathname.indexOf("runtime/previewpage") === 0 || reg.test(referrerPathname)) {
                        var jsonDirectory = [];
                        var jsonString = "";
                        if ($.cookie("pathNameDirectory_" + languageId) != undefined) {
                            jsonString = $.cookie("pathNameDirectory_" + languageId);
                            jsonDirectory = $.parseJSON(jsonString);
                        }
                        for (var i = 0; i < jsonDirectory.length; i++) {
                            var item = jsonDirectory[i];
                            if (item.pathName == referrerPathname && item.title != homeItemTitle && _self.options.isHideTitle == false) {
                                var html = "";
                                var splitIcon = $("#" + _self.options.controlId + " .w-crumbs i:first");
                                if (splitIcon.length > 0) {
                                    html += '&nbsp;<i class="' + $(splitIcon).attr("class").replace("f-hide", "") + '">' + $(splitIcon).html() + '</i>';
                                }
                                html += '&nbsp;<a href="/' + item.pathName + '" class="w-crumbs-item">' + item.title + '</a>';
                                $("#" + _self.options.controlId + " .w-crumbs a:first").after(html);
                                _self.smrecompute(hisHeight);
                                break;
                            }
                        }
                    }
...
+ function ($) {
    ...
    wzComment = function () {
        ...
        this.options = {
            ...
            commentTextMaxLength: "评论内容最长支付120个字符",
            replyTextEmpty: "请输入回复内容",
            replyTextMaxLength: "回复内容最长支付120个字符",
            captchaEmpty: "请输入验证码",
            captchaError: "验证码错误",
            error: "提交的数据有错误",
                ...
                success: function (result) {
                    if (result.IsSuccess) {
                        ...
                        for (var i = 0; i < list.length; i++) {
                            ...
                            html += '               <div class="w-comment-cont"><textarea name="replyText" rows="3" maxlength="120" placeholder="' + self.options.replyText + ' ' + item.CommentUserName + '："></textarea> <span class="words"><span>0</span>/120</span></div>';
        ...
        this.checkPostData = function (data) {
            ...
            if ($.trim(data.CommentText) == "") {
                ...
                if (data.CommentText.length > 120) {
                    if (type == "comment") {
        ...
        this.showError = function (target, message) {
            ...
            var prev = $(this.options.eventSource).parent().prev();
            var left = 0;
        ...
        this.checkPostData = function (data) {
            ...
            if ($.trim(data.Email) == "") {
                ...
                if (!$.wezhan.isEmail(data.Email)) {
                    this.showError(this.options.$emailElement, this.options.correctMailPrefix + ' ' + this.getElementTitle(this.options.$emailElement));
                    return false;
                }
            }
...
function recordPageAccess(url, title, pid, sid, eid, type) {
    $.ajax({
        url: "/PageVisit/Index",
        data: {
            UrlPath: url,
...
//针对图片控件的初始化
function InitImageSmv(controlId, width, styleHeight, fillType) {
    ...
    if (fillType == "1") {
        ...
        var dst = {};
        dst = $("#div_" + controlId + " img").FixFill(parseInt($control.css('width')), realHeight, "");
...
function SetCategoryStyleForPc(catId) {
    ...
    var categoryStr = localParam.substr(localParam.indexOf("=") + 1);
    if (categoryStr != "") {
...
function setCurrentPageTitle(pagetitle, languageId) {
    ...
    var reg = RegExp(/^\w*[a-zA-Z]+\w*$/);
    if (pathName.indexOf("runtime/previewpage") === 0 || reg.test(pathName)) {
        var jsonDirectory = [];
        var jsonString = "";
        var pageModel = { pathName: pathName, title: pagetitle };
        if ($.cookie("pathNameDirectory_" + languageId) == undefined) {
            jsonDirectory.push(pageModel);
        }
        else {
            jsonString = $.cookie("pathNameDirectory_" + languageId);
            jsonDirectory = $.parseJSON(jsonString);
        ...
        jsonString = JSON.stringify(jsonDirectory);
        $.cookie("pathNameDirectory_" + languageId, jsonString, { expires: 7, path: '/' });
    }
                ...
                success: function (result) {
                    if (result.IsSuccess) {
                        ...
                        if (href.indexOf("deviceModel=" + oldType) > -1) {
                            href.replace("deviceModel=" + oldType, "deviceModel=" + type);
                        }
